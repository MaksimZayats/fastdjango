...
from specx.testing.architecture.context import ArchitectureContext, class_base_name_index
    ...
    def check(self, context: ArchitectureContext) -> tuple[SpecxArchitectureViolation, ...]:
        ...
        for path in (context.src_root / "core").glob("*/use_cases/**/*.py"):
            ...
            use_cases = [
                ...
                if isinstance(node, ast.ClassDef) and is_use_case_class(node, aliases, base_index)
            ]
            if len(use_cases) != 1:
                violations.append(
                    violation(
